...
::: notes
Sales (Y) vs ad spend (X) by channel: TV, Radio, Newspaper.
Each dot = one market/time period (one observation).
TV shows strongest + relationship; Radio weaker/noisier; Newspaper ~ weak.
Point: univariate views are helpful, but predictors may overlap—eventually we model jointly.
:::
...
- **Units:** MSE is in **(units of $Y$)$^2$**; RMSE is in the **same units as $Y$**
...
::: notes
Say: “This is two plots about the same set of models.”
Left: same data, different flexibility.
Right: performance of those same models on training vs test.
Key message: training error keeps improving, but test error doesn’t.
We care about the red curve (generalization), not the gray curve (fit-to-history).
:::
...
![](img/overfit2.png){width=100% style="max-height:40vh; object-fit:contain; display:block; margin:0 0 0.4rem 0;"}
![](img/overfit3.png){width=100% style="max-height:40vh; object-fit:contain; display:block; margin:0;"}

:::
::::

::: notes
Now we’re seeing the same story three times:

- Each row/column is a different underlying “true” relationship / noise level.
- LEFT: points + candidate models (rigid → flexible).
- RIGHT: training error (gray) keeps dropping; test error (red) is U-shaped.

Why three?
- The location of the best flexibility (red minimum) depends on:
  - how nonlinear the true relationship is
  - how noisy the data is
  - sample size

Punchline: there’s no universally best flexibility—choose it using out-of-sample thinking.
:::
